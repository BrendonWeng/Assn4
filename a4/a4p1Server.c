...
#define PARMPTR ((struct serverParm *) parmPtr)
    ...
    {   
        //display infos
         printf("This server's ID is: %d\n",pthread_self());
         system("hostname; date");        
        
        recievedMsgLen[messageBuf] = '\0';
        //prints command
        char* printTHIS = (char*)malloc(4096);
        char minusN[4096]; 
        strncpy(minusN, messageBuf, 4096);
        remove_n(minusN);
        printf("Command: %s\n",messageBuf);           
        ...
        ///PROCESSING
             ...
             char* prev = (char*)malloc(4000);
             
             for(i=0; i < size; i++) {
                
                prev = concat(prev,list[i]);
                
             }
              if (write(PARMPTR->connectionDesc,concat(prev,"\n"),9000) < 0) {
